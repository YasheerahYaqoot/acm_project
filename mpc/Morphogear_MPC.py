...
N = 3       # Prediction horizon (Prediction time is N x Ts)
tolerance = 2
...
def xy_trajectories(option):
    if option == 1:
        ...
        #--------------------Option 4 - A* Trajectory Points
        x_values = [0, 0, 0, 100, 200, 300, 400, 400, 400, 500, 500, 600, 600, 700, 800, 800, 800, 900]
        y_values = [0, 100, 200, 200, 200, 200, 200, 300, 400, 400, 500, 500, 600, 600, 600, 700, 800, 800]
...
for index, (x_0, y_0) in enumerate(zip(x_values, y_values)):
    ...
    if abs(dy) > tolerance and abs(dx) <= tolerance:
        # -------------------Defining Rrobot_heightS Along Y-Axis
        Rrobot_heightS_Y = ca.vertcat(
                          a*np.sin(theta) - a*np.sin(theta), 
                          a*np.cos(theta) + a*np.cos(theta)
                          )
        f = ca.Function('f', [states, controls], [Rrobot_heightS_Y])
        if dy>0:
            Along_positive_Y = True
        elif dy<0:
            Along_negative_Y = True
    elif abs(dx) > tolerance and abs(dy) <= tolerance:
        # -------------------Defining Rrobot_heightS Along X-Axis
        Rrobot_heightS_X = ca.vertcat(
                          a*np.cos(theta) + a*np.cos(theta),
                          a*np.sin(theta) - a*np.sin(theta)
                          )
        f = ca.Function('f', [states, controls], [Rrobot_heightS_X])
        if dx>0:
    ...
    while (np.linalg.norm((x0-xs),2)) > tolerance and mpciter < sim_time/Ts:
        ...
        ff_value = ff(u_sol, args['p']) 
        xxl.append(ff_value)
        u_cl.append(DM2Arr(u_sol[:,0]))
        t.append(t0)
        t0, x0, u0 = shift_movement(Ts, t0, x0, u_sol, f)
...
for step_length, control_state in zip(U_Robot, tempstate):
    print(step_length, control_state)
    print(' ')

# ani_2D = simulation_2D(X_Robot, Y_Robot, U_Robot, tempstate, trajectory_option, figure_number=1, Repeat=False)
...
simulation_legs_2D(Morphogear_data, t1_FL, t2_FL, t1_BR, t2_BR, t1_FR, t2_FR, t1_BL, t2_BL, step)
